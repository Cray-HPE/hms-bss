...
"""
Test case for bootparameters update
"""
import sys
import bsslib
...
#
################################################################################
def updatebootparameters():
    "Test bootparameters update"
    test = "updatebootparameters"
    ret = True
    testname = "["+test+"]"
    ...
    if excode != 0:
        ...
        ret = False
    if ret:
        excode, result, errstr = run_bss(["bootparameters", "update",
                                          "--hosts", TESTDATA["host1"],
                                          "--params", TESTDATA["newparams"]])
        if excode != 0:
            print(testname+" FAIL: update fail: "+errstr)
            ret = False
        elif result is not None:
    ...
    if ret:
        print(testname+" PASS: Update successful")
        bsslib.cleanup(["host1", "kernel", "initrd", "newparams"])
    return ret

def test_updatebootparameters():
    "test for bootparameters update"
    assert updatebootparameters()

if __name__ == "__main__":
    sys.exit(updatebootparameters())
